...
$edit_staff = array();
$edit_staff['USER_ID'] = $_SESSION['edit_staff_id'];
...
if($username != $og_username) {
  $edit_staff['USERNAME'] = $username;
}
if($password != $og_password) {
  $edit_staff['USER_PASSWORD'] = $password;
}
if($firstname != $og_firstname) {
  $edit_staff['FIRST_NAME'] = $firstname;
}
if($lastname != $og_lastname) {
  $edit_staff['LAST_NAME'] = $lastname;
}
if($role != $og_role) {
  $edit_staff['USER_RANK'] = $role;
}
if($address1 != $og_address1) {
  $edit_staff['U_ADDRESS_LINE_1'] = $address1;
}
if($address2 != $og_address2) {
  $edit_staff['U_ADDRESS_LINE_2'] = $address2;
}
if($postcode != $og_postcode) {
  $edit_staff['U_POSTCODE'] = $postcode;
}
if($email != $og_email) {
  $edit_staff['EMAIL'] = $email;
}
if($alt_contact_num != $og_alt_contact_num) {
  $edit_staff['ALTERNATIVE_NUMBER'] = $alt_contact_num;
}
if($contact_num != $og_contact_num) {
  $edit_staff['CONTACT_NUMBER'] = $contact_num;
}
if($mark_opt != $og_mark_opt) {
  $edit_staff['MARKETING_OPT_IN'] = $mark_opt;
}

$url = "http://web.socem.plymouth.ac.uk/intProj/PRCS252A/api/user_account/" . $_SESSION['edit_staff_id'];
$encoded_edit = json_encode($edit_staff);
$string_edit = http_build_query($edit_staff);

echo $encoded_edit . "<br>";

$curl_options = array(
  CURLOPT_URL => "http://web.socem.plymouth.ac.uk/intProj/PRCS252A/api/user_account/" . $_SESSION['edit_staff_id'],
